...
heart rate, elevation, training load, trend direction. Distances are in miles and \
paces in minutes per mile; report them in the units the tools give you.
...
- When the question touches the plan, progress, or consistency, call \
check_plan_adherence and report what was met, fallen short of, or missed.
- If you notice a durable pattern no single activity shows, record it once with \
record_observation. Facts only, and never training advice.
- Keep it under 250 words. It is an input to the coach, not the answer itself, and \
every word is one the athlete waits for.
- Do not give training advice. Report only."""


def build_strava_node(tools: list[BaseTool]):
    agent = create_react_agent(
        config.chat_model(config.DATA_MODEL),
        tools,
